...
{
    public class FirstApiDataProvider : IApiDataProvider
    {
        ...
        public async Task<List<Route>> SearchAsync(SearchRequest request, CancellationToken cancellationToken)
        {
            ...
            var searchRequest = _mapper.Map<ProviderOneSearchRequest>(request);
            if (request.Filters is not null)
            {
                searchRequest.MaxPrice = request.Filters.MaxPrice;
                searchRequest.DateTo = request.Filters.DestinationDateTime;
            }
            ...
            var searchResponse = await JsonSerializer.DeserializeAsync<ProviderOneSearchResponse>(stream);
